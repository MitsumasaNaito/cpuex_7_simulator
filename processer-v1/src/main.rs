...
/// シミュレータの設定
//　ここで定義してdefault()で呼び出せるようにすることで、設定の変更が容易になり、拡張性のも上がる
#[derive(Debug, Clone)]
...
impl MipsSimulator {
    ...
    pub fn load_program_from_file<P: AsRef<Path>>(&mut self, path: P) -> Result<(), SimulatorError> {
        ...
        for (line_num, line) in reader.lines().enumerate() {
            let line = line.map_err(|e| SimulatorError::FileError(format!("行 {} を読み込めません: {}", line_num + 1, e)))?;
            let line = line.trim();
            ...
            if hex_part.is_empty() {
                continue; // 16進数部分が空の場合はスキップ
            }
            
            // 16進数として解析
            let instruction = u32::from_str_radix(hex_part, 16)
                .map_err(|e| SimulatorError::ParseError(format!("行 {}: 無効な16進数 '{}': {}", line_num + 1, hex_part, e)))?;
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            arg => {
                ...
                if arg.starts_with('-') {
                    return Err(format!("未知のオプション: {}", arg));
                }
...
fn main() {
    ...
    let program_file = match program_file {
        Some(file) => file,
        None => {
            eprintln!("エラー: プログラムファイルが指定されていません");
            print_usage();
            std::process::exit(1);
        }
    };
